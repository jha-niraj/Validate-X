...
function MediaValidationComponent({ post, onUpdate }: MediaValidationProps) {
	...
	const mediaUrls = post.mediaUrls || (post.fileUrl ? [post.fileUrl] : [])
	const isVideo = post.fileType?.startsWith('video/') || false
	...
	return (
		<div className="space-y-6">
							...
							<div className="flex items-start justify-between">
								<div className="flex-1">
									<div className="flex items-center gap-2 mb-2">
										...
										<Badge variant="secondary">{post.mediaSubtype}</Badge>
			...
			{/* Media Gallery & Validation Section */}
			<motion.div
				initial={{ opacity: 0, y: 20 }}
				animate={{ opacity: 1, y: 0 }}
			...
			>
						...
						<CardContent>
							{mediaUrls.length > 0 ? (
								<div className="space-y-4">
									{/* Main Media Display */}
									<div 
										className="relative bg-gray-50 dark:bg-gray-800 rounded-lg overflow-hidden cursor-crosshair"
				...
				<div>
					<Card>
						<CardHeader>
							<CardTitle className="text-lg">Validate This Media</CardTitle>
							<CardDescription>
							...
							<Tabs value={validationMode} onValueChange={(value) => setValidationMode(value as 'normal' | 'detailed')}>
								...
								<TabsContent value="detailed" className="space-y-4 mt-4">
									...
									<div className="space-y-2">
										...
										<Textarea
											placeholder="Provide comprehensive feedback on the media quality, composition, relevance, etc..."
									...
									{annotations.length > 0 && (
										<div className="space-y-2">
											<label className="text-sm font-medium">Annotations ({annotations.length}):</label>
											<div className="space-y-1 max-h-32 overflow-y-auto">
												{annotations.map((annotation, index) => (
													<div key={annotation.id} className="text-xs p-2 bg-gray-50 dark:bg-gray-800 rounded">
														<span className="font-medium">Point {index + 1}:</span> {annotation.comment}
													</div>
												))}
											</div>
										</div>
									...
									<div className="text-xs text-muted-foreground p-3 bg-blue-50 dark:bg-blue-950 rounded">
										💡 Tip: Click on the media to add annotations and provide specific feedback on different areas.
									</div>
...
export default MediaValidationComponent
